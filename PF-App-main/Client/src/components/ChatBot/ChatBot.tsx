...
const ANSWER: AnswerType = {
    ...
    Info: <p>Esta es una página de venta de productos relacionados con la computación</p>,
    Compra: <p>¿Deseas realizar una compra? Por favor, agregue su producto al carrito, y puede comprarlo a través de mercado pago.</p>,
    Usuario: <p>Como usuario puedes loguearte a través del register, o el login de Google. Además, puedes ver y comprar los productos con los que cuenta la página</p>,
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        setMessages((messages) =>
            messages.concat({
                id: String(Date.now()),
                type: "bot",
                text: ANSWER[response[0] as keyof typeof ANSWER] || ANSWER["Default"]
            })
        );
        setLoading(false);
            ...
            <div className="caja-chat">
                <div ref={container} className="chat">
                    {messages.map((message) => (
                        <div className={`chat-text ${message.type === "bot" ? "bg-primary rounded-pill me-auto text-light" : "bg-info rounded-pill ms-auto"} d-inline-block mb-3`} key={message.id}>
                            {message.text}
                        </div>
                    ))}
                </div>
                <form className="chat-form" onSubmit={handleSubmit}>
                    <input
                        ...
                        placeholder="Hace tu pregunta" className="barra" type="text" name="question" />
                    <button disabled={loading} className={'chat-btn ${loading ? "bg-primary":"bg-info"}'} type="submit" >Send</button>
                </form>
            </div>
        </div>
    )
